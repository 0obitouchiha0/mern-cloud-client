const SET_FILES = 'SET_FILES'
const SET_CURRENT_DIR = 'SET_CURRENT_DIR'
const ADD_FILE = 'ADD_FILE'
const PUSH_IN_STACK = 'POP_FROM_STACK'
const DELETE_FILE = 'DELETE_FILE'
const SET_VIEW = 'SET_VIEW'

const initialState = {
    files: [],
    currentDir: null,
    dirStack: [],
    view: 'list'
}

export default function(state = initialState, action) {
    switch(action.type) {
        case SET_FILES:
            return {
                ...state,
                files: action.payload
            }
        case SET_CURRENT_DIR:
            return {
                ...state,
                currentDir: action.payload
            }
        case ADD_FILE:
            return {
                ...state,
                files: [...state.files, action.payload]
            }
        case PUSH_IN_STACK:
            return {
                ...state,
                dirStack: [...state.dirStack, action.payload]
            }
        case DELETE_FILE:
            return {
                ...state,
                files: [...state.files.filter(file => file._id !== action.payload)]
            }
        case SET_VIEW:
            return {
                ...state,
                view: action.payload
            }
        default:
            return state
    }
}

export const setFiles = files => ({type: SET_FILES, payload: files})
export const setCurrentDir = id => ({type: SET_CURRENT_DIR, payload: id})
export const createFile = file => ({type: ADD_FILE, payload: file})
export const pushInStack = id => ({type: PUSH_IN_STACK, payload: id})
export const deleteFileAction = id => ({type: DELETE_FILE, payload: id})
export const setView = view => ({type: SET_VIEW, payload: view})